...
(function (global) {
	...
	//Agrega eventListener al objeto, al stirng y a la funcion
	...
	//Obtiene elementos DOMRect relative 
	var rect = function (e) {
		var t = win,
	...
	// Funcion para el paquete de cartas
	function Pack() {
		this.cards = [];
	...
	Game.prototype.mousedown = function (e) {
		...
		if (t && t.card) {
			...
			//automaticamnete agarra las cartas y las pone en la parte de arriba
			if (next) {
	...
	Game.prototype.updateHistory = function (card, start, end, siblings) {
		...
		if (Array.isArray(card)) {
			...
			obj = {
				card: card, // La carta que se mueve
				start: start, // La columna de origen
				end: end, // La columna destino
				siblings: siblings // alguna hermana
			};

			if (prev) {
				obj.prevSibling = {
					card: cards[prev.idx],
					flipped: cards[prev.idx].flipped // Estaba escondida?
				};
			}
		}
		...
		//Si el numero de movimientos almacenados excede el limite
		// remueve el movimiento más viejo hsata que este al maximo 
	...
	Game.prototype.updateScore = function (start, stop) {
		...
		// Mueve al los reyes de la columna vacia a otra vacia
		if (
			this.dropCount === 0 &&
			this.activeCard.value === 13 &&
		...
		//Mover desde el stack a alguna columna
	...
	Game.prototype.start = function () {
		...
		for (var i = 0; i < 28; i++) {
			...
			/* voltea la carta si es la primera */
			if (start === current) {
				card.flip();
			}

			/* incrementa las columnas en las que se deja la carta */
			current++;

			/* el incremento empieza positivo */
			if (current === 7) {
				start++;
				current = start;
			}
	...
	Game.prototype.reset = function () {
		...
		if (this.won) {
			// cancela la animacion del final
			if (this.frame) {
